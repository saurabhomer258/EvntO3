...
public class BookingTable extends AppCompatActivity {
    ...
    private ArrayList<BookTableList> bookTableListArrayList = new ArrayList<>();
    ...
    private void initView() {
        dbbooktable = new Dbbooktable(this);
        if (isNetworkAvailable()) {
            AsyncPost asyncPost = new AsyncPost(BookingTable.this, new ResponseListener() {
                @Override
                public void onResponse(String result) {
                    ...
                    try {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            BookTableList bookTableList = new BookTableList();
                            bookTableList.setId(e.getInt("id"));
                            bookTableList.setName(e.getString("name"));
                            bookTableList.setDescription(e.getString("description"));
                            bookTableList.setPrice(e.getString("price"));
                            bookTableList.setImageUrl(e.getString("imageurl"));
                            bookTableList.setMinOrder(e.getString("minOrder"));
                            bookTableList.setMenu(e.getString("menu"));
                            dbbooktable.addContact(bookTableList);
                            bookTableListArrayList.add(bookTableList);
                        }
                    ...
                    bookTableAdapter.setOnClickListner(new BookTableAdapter.OnItemClickListener() {
                        ...
                        public void OnItemClick(int position) {
                            ...
                            intent.putExtra("menu",bookTableListArrayList.get(position).getMenu());
                            intent.putExtra("imageurl",bookTableListArrayList.get(position).getImageUrl());
            ...
            asyncPost.execute("https://ceoindotech.000webhostapp.com/bookingtable.php");
        } /*else {
